...
class MIMIC_CXR_Dataset(Dataset):
    def __init__(self , csv_path, tokenizer_path, img_dir = '/home/asellerg/data/inputs/mimic-cxr/gcs/inputs/tmp/files/', img_tokens = 32, seq_length = 512,voc_size = 32000, mode = 'Train',start = 0,text_type = 'blank'):
    ...
    def __getitem__(self, idx):
        ...
        Question  = "What are the findings for this frontal chest x-ray?"
        Anwser = sample['answer']
        
        ##### read image pathes #####
        img_path = os.path.join(self.img_root, sample['report_id'], sample['image_name'])
        img_path = img_path.replace('.jpg', '.dcm')
        dcm = pydicom.dcmread(img_path)
        # img = PIL.Image.open(img_path).convert('RGB')
        img = PIL.Image.fromarray(dcm.pixel_array).convert('RGB')
